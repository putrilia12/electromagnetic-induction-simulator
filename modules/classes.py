...
# mendefinisikan berbagai objek dalam permainan/simulasi dengan fungsionalitas umum seperti menggambar, deteksi tabrakan, dan gerakan.
class Object:
...
# memeriksa apakah kotak pembatas objek (rect) bertabrakan dengan posisi mouse yg diberikan (mouse_pos)
...
# memperbarui posisi objek dengan mengatur koordinat pusat atribut rect-nya ke daftar pos yg diberikan.
    def move(self, pos):
        self.rect.centerx = pos[0]
        self.rect.centery = pos[1]

#  untuk mengelola objek magnet dalam simulasi dengan memperbarui, menggambar medan magnet, dan merespons perubahan parameter.
class Magnet(Object):
...
# menghitung dan menyimpan dftar garis medan magnet berdasarkan intensitas medan (B)
...
# menghitung dan menyimpan batas area medan magnet batas area yg mencakup semua garis medan magnet
...
# memperbarui garis medan magnet dan batas area medan magnet.
    def update_magnet(self):
        self.save_magnetic_lines_rect()
        self.get_rectunion()

# menggambar garis medan magnet di layar jika field visible aktif     
...
# menanggapi peristiwa tombol keyboard untuk menampilkan atau menyembunyikan medan magnet
...
# mengecek apakah magnet berada di dalam coil berdasarkan perhitungan   
 ...
 # menggerakkan magnet relatif terhadap posisi pointer mouse dengan memeriksa apakah magnet berada dlm koil atau tidak
...
# menanggapi peristiwa tombol keyboard untuk mengubah intensitas medan magnet (B)
...
# menginisialisasi coil, menggambar komponennya, memperbarui propertinya, dan menangani perubahan berdasarkan masukan pengguna (misalnya, penyesuaian tinggi, jumlah lilitan)
class Coil(Object):
...
# menggambar setengah lingkaran bagian atas dan bawah dari kumparan
...
# menggambar setengah lingkaran bagian bawah dari kumparan
...
# memperbarui kumparan bagian gulungan dan lilitan
    def update_coil(self):
        self.save_coils_rect()
        self.get_rectunion()

# Mengubah parameter koil berdasarkan input keyboard (naik, turun, kiri, kanan), dan memeriksa tumbukan dengan objek magnet.
...
# menangani interaksi antara magnet batang dan kumparan, menghitung parameter seperti gaya elektromotif, induksi magnet
class PhysicsHandler:
    ''' Parameters meaning:
            B = magnetic induction of bar magnet [V]
            n = number of coils
            d = diameter of coil [m]
            l = lenght of coil [m]
            E = electromotive force [V]
    '''
    # menginisialisasi berbagai atribut yang mewakili parameter fisik.
...
# Mengurangi gaya elektromotif sebesar nilai delta kecil jika melebihi ambang tertentu.
...
# membuat satuan
    def write_parameters(self):
        ...
        for name, param in self.dic.items():    
            ...
            if name == 'Induksi Magnet': 
                text += 'T'
            elif name == 'GGL Induksi': 
                text += 'V'
            elif name == 'Jumlah Lilitan':
                pass
            else: 
                text += 'm'

            obj = font.render(text, True, (0, 0, 0))
 ...
 # Mengonversi dan memperbarui nilai atribut untuk ditampilkan di layar (misalnya, cm ke meter, mT ke T).   
    def update_dict_attributes(self):
        d = self.d * pow(10, 2)
        l = self.l * pow(10, 2)
        B = self.B * pow(10, 3)
        E = self.E * pow(10, 3)
        self.dic['Jumlah Lilitan'] = self.n
        self.dic['Diameter Kumparan'] = round(d, 2)
        self.dic['Panjang Kumparan'] = round(l, 2)
        self.dic['Induksi Magnet'] = round(B, 2)
        self.dic['GGL Induksi'] = round(E, 2)

# Mengonversi nilai piksel menjadi  m
...
# Menghitung perubahan GGL selama interval waktu tertentu.     
...
# Memeriksa apakah terjadi gerakan relatif mouse untuk menentukan apakah simulasi harus dimulai atau dihentikan.
 ...
 # Memantau gerakan mouse untuk memulai, menghentikan, dan memperbarui simulasi, menghitung fluks magnetik dan gaya elektromotif.       
...
# mengontrol kekuatan cahaya
    def sigmoid(self, x):
        return 220 / (1 + exp(-x+8)) + 35

# Menyesuaikan warna bola lampu berdasarkan gaya elektromotif yang dihitung
